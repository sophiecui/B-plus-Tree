...
void BTNode :: setType(NODE_TYPE type) {
...
void BTInternal :: remove(int target) {
    ...
    all_keys[amount - 1] = NULL;
    all_pointers[amount] = NULL;
    amount--;
    return;
}
...
BTNode* BTLeaf :: getPointer(int i) {

    return NULL;
}
/*meanless for leaf node*/
void BTLeaf :: setPointer(int i, BTNode* pointer) {
...
void BTLeaf :: merge(BTLeaf* leaf) {
    ...
    assert(amount + leaf->getAmount() >= MAXNUM_KEY && "SUM OF AMOUNT is more thansupposed");
